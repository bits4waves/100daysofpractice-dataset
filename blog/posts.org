...
Now, let’s fetch the handle for the post.
This handle will be necessary to fetch the data.
To do this for the first shorcode we’ll need to run:
...
OK, now that we know how to grab the profile info, we can create a simple python script that will receive a shortcode and return the corresponding handle.
...
#+BEGIN_SRC python :tangle ../src/get-username.py
import argparse
import instaloader
...
Now, we’ll create a shell script to fetch the handle for each one of the shortcodes, and then fetch the data:

#+BEGIN_SRC shell :tangle ../src/get-usernames.sh
#!/bin/bash
